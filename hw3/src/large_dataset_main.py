...
def read_input():
    ...
    with open("../data/sampleInput.txt") as f:
        ...
        return target_user_id, target_movie_id, movie_ids, rating_dict, Rm, Ru, mu, movies
...
def main():
    ...
    target_user_id, target_movie_id, movie_ids, rating_dict, Rm, Ru, mu, movies = read_input()
    ...
    for key, val in rating_dict.items():
        ...
        u = int(key_array[0])
        m = int(key_array[1])
        rating_matrix[u][movie_ids.index(m)] = val
    ...
    for u_ in range(1, num_of_row):
        for m_ in range(1, num_of_col):
            ...
            if rating_matrix[u_][m_] == -1:
                ...
                for movie in Ru[u_]:
                    # if movie in b_m:
                    b_m_sum += b_m[movie_ids.index(movie)]
